## Homework Assignment 02 (10/15/2023)
...
#### 12. `rgb_to_hex`
Write a program which reads in three integers `r, g, b` representing RGB values ranging from `0` to `255` (inclusive) and prints out their hexadecimal representation. For example, if the input is `245 13 0` the output would be `#F50D00`. **Cannot use std::hex or any other built-in function(s) that solves the entire problem**.
